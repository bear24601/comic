(() => {
  ...
  function preloadNearbyPages() {
    [
      ...
      chapter.pages[currentPage - 1],
      chapter.pages[currentPage + 2]
    ].forEach(path => preloadImage(path));
  }

  function displayLoadedImage(path, preloadRef) {
    if (preloadRef) preloadedImages.set(path, preloadRef);
    comicImage.src = path;
  ...
  function showPage() {
    ...
    const cached = preloadedImages.get(targetPath);

    if (cached && cached.complete && cached.naturalWidth > 0) {
      displayLoadedImage(targetPath, cached);
    } else {
      const preload = new Image();
      preload.onload = () => displayLoadedImage(targetPath, preload);
      preload.onerror = () => {
        comicImage.hidden = true;
        loadingText.hidden = false;
        loadingText.textContent = `圖片載入失敗：${targetPath}`;
      };
